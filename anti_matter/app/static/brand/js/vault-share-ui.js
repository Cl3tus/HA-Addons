/**
 * Share dialog + card icon actions for AntiMatter vault UIs (cloud + HA).
 */
(function (global) {
  ...
  function bindShareUi(config) {
    ...
    async function downloadShareImage(code, codeId) {
      ...
      const proto =
        ...
        window.AntiMatterHomeKitPayload?.codeProtocol?.(code) ||
        "matter";
      const path =
      ...
      a.download =
        proto === "homekit"
          ? "antimatter-homekit.svg"
          : proto === "zwave"
            ? "antimatter-zwave.svg"
            : "antimatter-code.png";
      a.click();
    ...
    document.getElementById("share-btn-link")?.addEventListener("click", async () => {
      ...
      if (config.cloudShareEnabled === false) {
        alert(
          msg.cloudRequired ||
            "Configure AntiMatter Cloud and run Cloud sync to create secret share links."
        );
        return;
      }
      try {
